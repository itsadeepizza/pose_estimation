...
class MediaPipeProcessor():
    ...
    def get_gesture(self, handeness='Right'):
        if self.results.multi_hand_landmarks:
            for hand_landmarks, multi_handedness in zip(self.results.multi_hand_landmarks, self.results.multi_handedness):
                # Check if it the handeness matches ( I need to reverse the condition otherwise it does not work ??)
                if multi_handedness.classification[0].label != handeness:
                    probs = model.predict_proba(hand_landmarks, multi_handedness.classification[0].label)
                    return probs
        return None
    ...
    def update_overlayer(self):
        ...
        probs = self.rigth_gesture[-1]
        print(probs)
        # Show gesture probabilities as gauge bars
        if probs is not None:
            for i, (gesture, prob) in enumerate(probs.items()):
                cv2.rectangle(self.overlayer, (0, i*20), (int(prob*100), (i+1)*20), (255, 0, 0), -1)
                cv2.putText(self.overlayer, gesture, (0, (i+1)*20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1, cv2.LINE_AA)
class VideoCapture:
